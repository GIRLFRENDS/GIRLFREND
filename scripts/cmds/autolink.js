...
module.exports = {
  ...
  onStart: async function ({ api, event }) {
    ...
    if (event.body.toLowerCase().includes('autolink off')) {
      ...
      saveAutoLinkStates(autoLinkStates);
      api.sendMessage("⚠️  ʀʌʜʌt ɓot ʌʋtoɭɩŋĸ oʆʆ sʋccɘssʆʋɭ.", event.threadID, event.messageID);
    } else if (event.body.toLowerCase().includes('autolink on')) {
      autoLinkStates[threadID] = 'on';
      saveAutoLinkStates(autoLinkStates);
      api.sendMessage("✅ ʀʌʜʌt ɓot ʌʋtoɭɩŋĸ oŋ sʋccɘssʆʋɭ.", event.threadID, event.messageID);
    }
  },
